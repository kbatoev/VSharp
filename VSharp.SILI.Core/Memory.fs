...
// ------------------------------- Composition -------------------------------
    ...
    let private fillAndMutateStackLocation isEffect state stack (k : stackKey) p (v : term option) =
        match v with
        | Some v ->
            let k' = k.Map (typeVariableSubst state)
            let v' = fillHoles state v
            writeStackLocation stack k' v'
        | None when isEffect -> stack // already reserved
        | None -> reserveLocation stack k p // new frame, so we need to reserve
